...
public final class BlazeCWorkspace implements ProjectComponent {
  ...
  private List<String> collectIncludes(Path root, TargetKey targetKey, BlazeProjectData blazeProjectData) {
    ...
    for (ArtifactLocation header : cIdeInfo.getHeaders()) {
      ...
      String codePathStr = codePath.toString();

      if (includePrefix != null) {
        codePathStr = includePrefix + "/" + codePathStr;
      }

      includes.add("-ibazel" + codePathStr + "=" + realPath);
    }
  ...
  private List<String> collectIncludesWithProgress(
    ...
    logger.info(String.format("Collection includes took %dms", elapsed));
