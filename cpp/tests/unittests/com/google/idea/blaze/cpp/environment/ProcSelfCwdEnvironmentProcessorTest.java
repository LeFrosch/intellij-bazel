...
public class ProcSelfCwdEnvironmentProcessorTest extends BlazeTestCase {
  ...
  public void testRewritesProcSelfCwdValueToAbsolutePath() {
    ImmutableMap<String, String> result =
        processor.process(
            ImmutableMap.of("QNX_HOST", "/proc/self/cwd/external/qnx/host"), resolver);

    assertThat(result)
        .containsEntry("QNX_HOST", new File(OUTPUT_BASE, "external/qnx/host").getAbsolutePath());
  }

  @Test
  public void testLeavesNonPathValuesUntouched() {
    ImmutableMap<String, String> environment =
        ImmutableMap.of("FOO", "bar", "REL", "some/relative/value");
  ...
  public void testLeavesGenuineAbsolutePathsUntouched() {
    ImmutableMap<String, String> environment = ImmutableMap.of("PATH", "/usr/bin:/bin");
