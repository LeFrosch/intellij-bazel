...
class BazelExecServiceImpl(private val project: Project, private val scope: CoroutineScope) : BazelExecService {
  ...
  private fun resolveBinaryPath(): String {
    ...
    if (projectView != null) {
      val projectSpecificBinary = projectView.getScalarValue(BazelBinarySection.KEY).orElse(null)
      if (projectSpecificBinary != null) {
        return projectSpecificBinary.path
      }
    }
  ...
  override fun exec(ctx: BlazeContext, cmdBuilder: BlazeCommand.Builder): ExecResult {
    ...
    return executionScope(ctx) {
      ...
      val exitCode = execute(ctx, cmdBuilder) { text ->
        outputStream.write(text.toByteArray())
      }

      outputStream.close()

      ExecResult(exitCode, tempFile)
    }
  }
}
